...
import egovframework.rte.bat.core.item.file.mapping.EgovByteLineMapper;
 ...
 * </pre>
 */
public class EgovFlatFileByteReader<T> extends
	...
	private byte[] readLine() {
		...
		try {
			...
			noInput = true;
			throw new NonTransientFlatFileException(
					"Unable to read from resource: [" + resource + "]", e,
					line + "", lineCount);
		}
		return b;
	}
	...
	protected void doOpen() throws Exception {
		...
		Assert.notNull(recordSeparatorPolicy,
				"RecordSeparatorPolicy must be set");

...
public class EgovPartitionFlatFileItemWriter<T> extends
		ExecutionContextUserSupport implements
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(EgovPartitionFlatFileItemWriter.class);
	...
	private void doOpen(ExecutionContext executionContext)
		...
		try {
			...
			if ((fileOpenTime - fileCloseTime) / TIME_PERCENT < 1) {
...
//							});
					...
					TransactionAwareBufferedWriter tabw = new TransactionAwareBufferedWriter(fileChannel,
							new Runnable() {
								public void run() {
									closeStream();
								}
							});
					tabw.setEncoding(encoding);
					return tabw;

...
import egovframework.rte.bat.core.item.file.EgovFlatFileByteReader;
...
public class EgovFixedByteTokenizer extends EgovAbstractByteLineTokenizer {
	 ...
	 * 
	 * @param line
	 *            토큰의 대상이 되는 문자열 (can be <code>null</code>)
	...
	protected List<String> doTokenize(byte[] byteString, String encoding)
			throws Exception {
		String token;
		...
		if (lineLength == 0) {
			throw new IncorrectLineLengthException(
					"Line length must be longer than 0", maxRange, lineLength);
		}

		if (lineLength < maxRange) {
			throw new IncorrectLineLengthException(
					"Line is shorter than max range " + maxRange, maxRange,
					lineLength);
		}

		if (!open && lineLength > maxRange) {
			throw new IncorrectLineLengthException(
					"Line is longer than max range " + maxRange, maxRange,
					lineLength);
		}

...
public class LoggerHierarchyTest {
	...
	public void testLoggerHierarchy() throws Exception {
		...
		// Logger targetLogger = LogManager.getLogger("egovframework.rte.fdl.logging.sample.LogTestSample"); 과 동일
		...
		assertEquals("egovframework.rte.fdl.logging.sample.LogTestSample", targetLogger.getName()); 
		
		// targetLogger의 ParentLogger가 egovframework 로거인지 확인
		Logger parentLogger = targetLogger.getParent();
		Logger egovLogger = (Logger) LogManager.getLogger("egovframework");
		...
		// 로그 출력
		// 현재 egovframework 로그 레벨이 log4j2.xml 에 DEBUG 로 정의돼 있으므로

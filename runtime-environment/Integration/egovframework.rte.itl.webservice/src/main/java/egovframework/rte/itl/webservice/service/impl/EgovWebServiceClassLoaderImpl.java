...
public class EgovWebServiceClassLoaderImpl extends ClassLoader implements
	...
	public static final String CLASS_NAME_PREFIX = BASE_PACKAGE_NAME + "."
			+ "EgovType";
	...
	public static final String DESC_OF_SERVICE_BRIDGE_CLASS = "L"
			+ NAME_OF_SERVICE_BRIDGE_CLASS + ";";
	...
	public Class<?> loadClass(final Type type) throws ClassNotFoundException {
		...
		if (type instanceof PrimitiveType) {
			...
			try {
				LOGGER.debug("Check the class \"{}\" is already loaded.",
						className);
	...
	public Class<?> loadClass(final ServiceEndpointInfo serviceEndpointInfo)
		...
		LOGGER.debug("loadClass of ServiceEndpointInfo ({})",
				serviceEndpointInfo);
	...
	public Class<?> loadClass(
			...
			throws ClassNotFoundException {
		LOGGER.debug("loadClass of ServiceEndpointInterfaceInfo ({})",
	...
	private byte[] createRecordClass(final String className,
		...
		// Create Annotation
		AnnotationVisitor annotationVisitor = classWriter.visitAnnotation(
				DESC_OF_XML_ACCESSOR_TYPE, true);
	...
	private byte[] createServiceEndpointInterfaceClass(
			...
			throws ClassNotFoundException {
		// CHECKSTYLE:OFF
		String serviceEndpointInterfaceClassName = getServiceEndpointInterfaceClassName(serviceEndpointInterfaceInfo
				.getServiceName());

		String asmServiceEndpointInterfaceClassName = serviceEndpointInterfaceClassName
				.replace('.', '/');
		// CHECKSTYLE:ON
		...
		// Create Annotation
		AnnotationVisitor annotationVisitor = classWriter.visitAnnotation(
				DESC_OF_WEB_SERVICE, true);
		annotationVisitor.visit("targetNamespace",
				serviceEndpointInterfaceInfo.getNamespace());
		annotationVisitor.visitEnd();
		annotationVisitor = classWriter.visitAnnotation(DESC_OF_SOAP_BINDING,
				true);
		...
		// @WebMethod
		annotationVisitor = methodVisitor.visitAnnotation(DESC_OF_WEB_METHOD,
				true);
		annotationVisitor.visit("operationName",
				serviceEndpointInterfaceInfo.getOperationName());
		annotationVisitor.visitEnd();

		// @WebResult
		if (returnInfo != null) {
			annotationVisitor = methodVisitor.visitAnnotation(
					DESC_OF_WEB_RESULT, true);
		...
		int index = 0;
		for (ServiceParamInfo info : serviceEndpointInterfaceInfo.getParamInfos()) {
			annotationVisitor = methodVisitor.visitParameterAnnotation(index,
	...
	private byte[] createServiceEndpointClass(
			...
			throws ClassNotFoundException {
		String serviceEndpointInterfaceClassName = getServiceEndpointInterfaceClassName(serviceEndpointInfo
				.getServiceName());
		String serviceEndpointClassName = getServiceEndpointClassName(serviceEndpointInfo
				.getServiceName());
		...
		annotationVisitor.visit("targetNamespace",
				serviceEndpointInfo.getNamespace());
		// annotationVisitor.visit("name",
		// serviceProviderInfo.getServiceName());
		annotationVisitor.visit("serviceName",

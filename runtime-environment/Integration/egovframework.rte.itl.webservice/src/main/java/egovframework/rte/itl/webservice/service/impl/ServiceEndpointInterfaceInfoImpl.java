...
public class ServiceEndpointInterfaceInfoImpl implements
	...
	public ServiceEndpointInterfaceInfoImpl(
		...
		// == false)
		// {
		// LOGGER.error("Argument 'webServiceClientDefinition.namespace' has "
		// +
		// "no text (" +
		// webServiceClientDefinition.getNamespace() +
		// ")");
		...
		// == false)
		// {
		// LOGGER.error("Argument 'webServiceClientDefinition.wsdlAddress' has "
		// +
		// "no text (" +
		// webServiceClientDefinition.getWsdlAddress()
		// +
		...
		// == false)
		// {
		// LOGGER.error("Argument 'webServiceClientDefinition.serviceName' has "
		// +
		// "no text (" +
		// webServiceClientDefinition.getServiceName()
		// +
		...
		// false)
		// {
		// LOGGER.error("Argument 'webServiceClientDefinition.portName' has "
		// +
		// "no text (" +
		// webServiceClientDefinition.getPortName() +
		// ")");
		...
		// == false)
		// {
		// LOGGER.error("Argument 'webServiceClientDefinition.operationName' "
		// +
		// "has no text (" +
		// webServiceClientDefinition.getOperationName()
		// + ")");
		...
		if (webServiceClientDefinition.getServiceDefinition().isStandard()) {
			...
			if (webServiceClientDefinition.getRequestMappingInfos() != null) {
				for (Entry<String, MappingInfo> entry : webServiceClientDefinition
					...
					ServiceParamInfoImpl paramInfo = new ServiceParamInfoImpl(
							fieldName, fieldType, Mode.IN,
			...
			if (webServiceClientDefinition.getResponseMappingInfos() != null) {
				for (Entry<String, MappingInfo> entry : webServiceClientDefinition
					...
					ServiceParamInfoImpl paramInfo = (ServiceParamInfoImpl) paramInfoMap
							.get(fieldName);
					if (paramInfo != null) {
					...
					if (paramInfo == null) {
						paramInfo = new ServiceParamInfoImpl(fieldName,
								fieldType, Mode.OUT, mappingInfo.isHeader());
						paramInfoMap.put(fieldName, paramInfo);
					}
				}
			}
